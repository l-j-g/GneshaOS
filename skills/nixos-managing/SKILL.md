...
<!-- Adapted from https://github.com/michalzubkowicz/nixos-management-skill -->

## Repository safety rules

This reference is supplemental to this repository's `AGENTS.md` and `gneshaos-maintenance` skill. Follow those first whenever guidance differs. The examples below are general references; inspect this repository and its pinned NixOS/Home Manager versions before applying any pattern.

- Keep this repository's machine unchanged unless the user explicitly requests activation. Use build-only validation by default. Do not run `sudo`, `nixos-rebuild switch`, `nixos-rebuild boot`, installation, partitioning, TPM/LUKS enrollment, or destructive Nix store commands without explicit approval.
- Treat `nixos-rebuild test`, rollback commands, disk formatting, filesystem/subvolume deletion, LUKS keyslot changes, and remote deployments as state-changing operations. Get explicit approval before running them.
- Never alter generated `hosts/*/hardware-configuration.nix` by hand. Do not update `flake.lock` unless the user asks to update inputs.
- Do not copy example secrets, credentials, hostnames, disk devices, filesystem layouts, or service settings into this machine. Verify option paths against the repository's locked inputs and inspect local module composition first.
- Do not install or configure optional MCP servers or external tools as a side effect of using this skill. Review and ask separately before adding them.
