...
| `systemSettings.airVpn.configPath` | User-selected path to a root-owned mode-600 WireGuard profile outside the repo and Nix store. Set it in `system-parameters.nix`; the service validates it and its parent directories. | `"/etc/airvpn/host.conf"` on this host | system AirVPN module |
